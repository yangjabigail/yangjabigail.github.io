...
function test_next_row_function(row) {
    return row; 
}
function test_next_col_function(col) {
    return col; 
}
                                                  // Pull these names into this module's scope for convenience:
const { vec3, vec4, color, Mat4, Light, Shape, Material, Shader, Texture, Scene } = tiny;
const { Triangle, Square, Tetrahedron, Windmill, Cube, Subdivision_Sphere, Grid_Patch } = defs;

export class Test_Scene_Base extends Scene
...
export class Test_Scene extends Test_Scene_Base
  ...
  display( context, program_state )
    {                                                // display():  Called once per frame of animation.  For each shape that you want to
      ...
      **********************************/         
      ...
      const blue = color( 0,0,1,1 ), yellow = color( 1,1,0,1 ), green = color( 0,1,0,1 );
      ...
      this.shapes.triangle.draw( context, program_state, Mat4.identity(), this.materials.plastic.override( green ) );
